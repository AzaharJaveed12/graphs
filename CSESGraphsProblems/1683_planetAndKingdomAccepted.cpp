...
void createWeightedGraphFromEdges(int V,bool isDirected) {
...
void secondDFS(int u,int &componentCount,VB &visited,VI &components) {
    ...
    for(int v:graph[u]) {
        if(!visited[v]) secondDFS(v,componentCount,visited,components);
    }
}

void kosrajuSSCAlgorithm(int n) {
    stack<int> stk;
    ...
    LOOP(i,0,edges.size(),1) swap(edges[i][0],edges[i][1]);
