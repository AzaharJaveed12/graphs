...
Identified by : need the no of disconnected components and max. component number every time we insert a edge.
                Dynamically edges are getting added.
So This is : so this is UFDS based problem.

*/

class UFDS {
private:
    VI parent,rank,componentSize;
    int totalComponents,maxCompSize;
    int findRepresentative(int u) {
        if(parent[u] == u) return u;
        return parent[u] = findRepresentative(parent[u]);
    }
    ...
    void mergeComponents(int u, int v) {
        int parentU = findRepresentative(u);
        int parentV = findRepresentative(v);
        
        if(rank[parentU] < rank[parentV]) {
            parent[parentU] = parentV;
            componentSize[parentV] += componentSize[parentU];
        }else {
            parent[parentV] = parentU;
            componentSize[parentU] += componentSize[parentV];
...
public:
    ...
    void addEdge(int u,int v) {
        if(!inSameComponent(u,v)) {
            mergeComponents(u,v);
            totalComponents--;
            maxCompSize = max(maxCompSize,max(componentSize[findRepresentative(u)],componentSize[findRepresentative(v)]));
        }
    }

    II getSolution() {
        return {totalComponents,maxCompSize};
    }
};
