...
Identified by : Need to check whether all nodes can reach all nodes.   
                --> by simplifying this :
                    can 1 reach n - 1 node  && n-1 nodes reach 1 --> all nodes can reach from & to 1.
So This is : Simple DFS with from and to visited arrays will work here.
...
int main() {
    ...
    VB visitedFrom(n + 1 , false),visitedTo(n + 1 ,false);
    DFS(1,visitedFrom);
    LOOP(i, 2 ,n + 1, 1) {
        if(visitedFrom[i] == false) {
            cout<<"NO\n";
            cout<<1<<" "<<i;
            return 0;
        }
    ...
    LOOP(i,0,m,1) swap(edges[i][0],edges[i][1]);
    createGraphFromEdges(n,edges,true);
    DFS(1,visitedTo);

    LOOP(i, 2 , n + 1, 1) {
        if(visitedTo[i] == false) {
            cout<<"NO\n";
            cout<<i<<" "<<1;
            return 0;
        }
