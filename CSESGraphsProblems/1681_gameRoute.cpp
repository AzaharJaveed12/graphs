...
void DFS(int u, VLL &ans) {
    if(ans[u] != -1) return;
    LL possibleWaysToReachNFromU = 0;
    for(int v:graph[u]) {
        DFS(v,ans);
     //   DEB3(ans[v],v,possibleWaysToReachNFromU);
        possibleWaysToReachNFromU += ans[v];
        possibleWaysToReachNFromU = possibleWaysToReachNFromU%(MOD7);
    }
    ans[u] = possibleWaysToReachNFromU;
}
int main() {
    int n,m;
    cin>>n>>m;
