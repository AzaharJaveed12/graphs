...
Identified by : need to form two teams such that no two persons are directly connected. 
So This is : Bibarted graph
    Reason :

*/

bool isBipartiateGraph(int u,VI &color,int clr) {
    if(color[u] != -1)  return color[u] == clr;
    color[u] = clr;
    bool isBipartiate = true;
    for(int v:graph[u]) {
        isBipartiate &= isBipartiateGraph(v,color,clr^1);
    }
    return isBipartiate;
}
...
int main() {
    ...
    VI color(n + 1, -1);
    LOOP(i,1,n + 1, 1) {
        if(color[i] == -1 && !isBipartiateGraph(i,color,0)) {
           // DEB1(i);
            cout<<"IMPOSSIBLE";
            return 0;
        }
